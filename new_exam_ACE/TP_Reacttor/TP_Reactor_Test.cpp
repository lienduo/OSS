...
MyTask::create_reactor (void)
{
        ...
        ACE_NEW_RETURN (my_reactor_,
                        ACE_Reactor (pImpl ,1),  //不在析构器中删除TP_Reactor
                        -1);

        ACE_DEBUG ((LM_DEBUG,
                                ACE_TEXT (" (%t) Create TP_Reactor\n")));

        //关键： 07-09-14  目的：替换原来的Reactor单体，使用新定义的TP_Reactor,便于后面直接使用ACE_Reactor::instance()方法
        ACE_Reactor::instance (this->my_reactor_);

        this->reactor (my_reactor_); //ACE_Task->ACE_Service_Objector->ACE_Event_Handler( reactor())
...
MyTask::svc (void)
{
        ...
        while (ACE_Reactor::event_loop_done () == 0)  //多个线程中运行事件检测 
...
Acceptor::on_new_receiver (Receiver &rcvr)
{
        ...
        void
Acceptor::on_delete_receiver (Receiver &rcvr)  //使用队列管理，好像没必要 
{
        ACE_Guard<ACE_Recursive_Thread_Mutex> locker (this->mutex_);

        this->sessions_--;   //如果仅仅需要限制连接数，那么用session_数就足够
...
Acceptor::start (const ACE_INET_Addr &addr)
{
        if (ACE_Acceptor<Receiver,ACE_SOCK_ACCEPTOR>
                        ::open (addr,
                                ACE_Reactor::instance (),//使用重定义的TP_Reactor
                                ACE_NONBLOCK) < 0)
                ACE_ERROR_RETURN ((LM_ERROR,
                                        "%p\n",
...
Receiver::open (void *)
{
        ...
        ACE_Reactor *TPReactor = ACE_Reactor::instance ();//使用重定义的TP_Reactor
...
int Sender::open (void *)
{
        ...
        int
Sender::initiate_write (void)  //发起写操作->handler_out()
{
        ...
        // ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("Sender:: ctreat write task\n")));
        return initiate_io (ACE_Event_Handler::WRITE_MASK); //检测写事件
}
...
{
        // ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("Sender:: send data\n")));
        ACE_Guard<ACE_Recursive_Thread_Mutex> locker (mutex_);
