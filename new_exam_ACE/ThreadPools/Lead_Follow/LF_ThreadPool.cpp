// LF_ThreadPool.cpp,v 1.3 2004/01/05 22:57:06 shuston Exp
...
int LF_ThreadPool::svc (void)
{
        ...
        while (!done ())
        {
                ...
                {
                        if (elect_new_leader () == 0)    // elect the leader OK
                                continue;  
                        break;       // no follower left. a thread want to become a leader,but not to wake up!
                }
...
int LF_ThreadPool::become_leader (void)
{
        ...
        ACE_GUARD_RETURN
                (ACE_Thread_Mutex, leader_mon, this->leader_lock_, -1);  
                         //only one thread wait for becoming leader.
                         //if the thread can't become a leader,other threads can't get the lock.
        if (leader_active ())
        {
                Follower *fw = make_follower ();
                {
                        ...
                        while (leader_active ())
                                fw->wait ();    //wait the same fw to wake up
                }
...
int LF_ThreadPool::elect_new_leader (void)
{
        ...
        else   //a thread want to become a leader,but not to wake up! other threads can get the lock!
        {
                ACE_DEBUG
