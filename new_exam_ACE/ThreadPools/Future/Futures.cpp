...
class Worker: public ACE_Task<ACE_MT_SYNCH>
{
        public:
                ...
                virtual int svc (void)
                {
                        ...
                        while (1)
                        {
                                ACE_Method_Request *request = this->queue_.dequeue();  //wait for request 
                                if (request == 0)
                                        return -1;

                                // Invoke(call) the request
                                int result = request->call ();   //LongWork::call(). if Exit,svn will be stop
                                if (result == -1)
                                        break;
