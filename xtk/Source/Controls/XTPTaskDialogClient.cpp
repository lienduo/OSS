...
void CXTPTaskDialogClient::LoadStrings()
{
	...
	if (!m_strExpandedInformation.IsEmpty())
	{
		...
		{
			VERIFY(XTPResourceManager()->LoadString(&m_strExpandedControlText, XTP_IDS_TASK_HIDE_DETAILS));
			VERIFY(XTPResourceManager()->LoadString(&m_strCollapsedControlText, XTP_IDS_TASK_SEE_DETAILS));
		}
