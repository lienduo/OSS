...
void CXTTabExBase::EnableRedraw(BOOL bRedraw)
{
...
BOOL CXTTabExBase::OnSelchange(NMHDR* pNMHDR, LRESULT* pResult)
{
	*pResult = 0;

	// This is here for backward compatibility, TCN_XT_SELCHANGE
	// message has been deprecated, please use TCN_SELCHANGE instead.
	CWnd* pOwner = m_pTabCtrl->GetOwner();
	if (::IsWindow(pOwner->GetSafeHwnd()))
	{
		*pResult = pOwner->SendMessage(TCN_XT_SELCHANGE,
...
BOOL CXTTabExBase::OnSelchanging(NMHDR* pNMHDR, LRESULT* pResult)
{
	*pResult = 0;

	// This is here for backward compatibility, TCN_XT_SELCHANGING
	// message has been deprecated, please use TCN_SELCHANGING instead.
	CWnd* pOwner = m_pTabCtrl->GetOwner();
	if (::IsWindow(pOwner->GetSafeHwnd()))
	{
		*pResult = pOwner->SendMessage(TCN_XT_SELCHANGING,
			pNMHDR->idFrom, (LPARAM)m_pTabCtrl);
	}
