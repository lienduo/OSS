...
CSize CXTPToolTipContext::CHTMLToolTip::GetToolSize(TOOLITEM* lpToolInfo)
{
	...
	if (m_wndBrowser.GetSafeHwnd() == 0)
	{
		...
		m_pBrowserApp->Navigate(_bstr_t(XTP_CT2CW(strBody)),
			(VARIANT*)&varEmpty,
			(VARIANT*)&varEmpty,
			(VARIANT*)&varEmpty,
			(VARIANT*)&varEmpty);
	...
	IDispatchPtr spDocument = NULL;
	m_pBrowserApp->get_Document(&spDocument);
	...
	IHTMLDocument2Ptr spHTML = NULL;
	spDocument->QueryInterface(IID_IHTMLDocument2, (void**)&spHTML);
	if (spHTML == 0)
		return 0;
	...
	IHTMLElementPtr spBody = NULL;
		...
		// Get the BODY object
	HRESULT hr = spHTML->get_body(&spBody);
	if (FAILED(hr) || (spBody == 0))
		return 0;
	...
	spBody->put_innerHTML(_bstr_t(XTP_CT2CW(strText)));

	m_wndBrowser.MoveWindow(0, 0, 650, 200);


	IHTMLTextContainerPtr spHTMLContainer = NULL;
	spBody->QueryInterface(IID_IHTMLTextContainer, (void**)&spHTMLContainer);

	if (spHTMLContainer == 0)
		return 0;

	long nScrollHeight, nScrollWidth;
	spHTMLContainer->get_scrollHeight(&nScrollHeight);
	...
	do
	{
		...
		spHTMLContainer->get_scrollHeight(&nScrollHeight);
		spHTMLContainer->get_scrollWidth(&nScrollWidth);
...
CWnd* CXTPToolTipContext::CreateToolTip(CWnd* /*pOwner*/)
{
	...
	case xtpToolTipOffice2007:
		return new COffice2007ToolTip(this);
		...
		GetKeyState(VK_MBUTTON) >= 0))
	{
		...
		if (bTipInfoChanged || CRect(m_lastInfo.rect) != CRect(tiHit.rect))
		{
			...
			if (nHit != -1)
			{
				...
				if ((m_toolStyle != xtpToolTipRTF && m_toolStyle != xtpToolTipHTML) && ((m_dwComCtlVersion >= VERSION_IE5) || (m_toolStyle != xtpToolTipStandard)))
				{
