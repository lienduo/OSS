...
XTP_IMAGERESOURCE_INFO* CXTPImageManagerResource::Add(CXTPImageManagerIconHandle& hHandle)
{
	CXTPLockGuard lock(m_cs);
	...
	{
		ICONINFO iconInfo;
		if (GetIconInfo(hHandle.m_hIcon, &iconInfo))
		{
			LPBYTE pBitsI = NULL;
			BOOL bAlpha = -1;
	...
	{
		LPBYTE pBitsI = hHandle.PreMultiply();
		if (!pBitsI)
			return NULL;
