...
void CXTPSyntaxEditCtrl::Draw(CDC *pDC, const CRect& rcRect)
{
	...
	for (int nLine = 0; nLine < nLinesCount; nLine++)
	{
		int nTextRow = m_nTopRow + nLine + nSkipLines;
		BOOL bSkipRow = FALSE;
		DWORD dwCollapcedType = 0;

//      // *** m_bDrawNodes
//      // TODO: if m_bDrawNodes is false - skip processing of collapsed lines. ???
//       expand all when set m_bDrawNodes = FALSE
		if (GetSyntaxColor())// m_bDrawNodes)
		{
			dwCollapcedType = ProcessCollapsedRowsBeroreDraw(nTextRow, ptrTxtSch, bSkipRow);
...
//       expand all when set m_bDrawNodes = FALSE
	if (GetSyntaxColor())// m_bDrawNodes)
	{
		ProcessCollapsedRowsBeroreDraw(nTextRow, ptrTxtSch, bSkipRow); // to update collapsed block info.
	}
...
DWORD CXTPSyntaxEditCtrl::ProcessCollapsedRowsBeroreDraw(int nTextRow, CXTPSyntaxEditLexTextSchema* pTxtSch, BOOL& rbSkipRow)
{
	// *** m_bDrawNodes
	// TODO: if m_bDrawNodes is false - skip processing of collapsed lines. ???
	...
	if (!pTxtSch || nTextRow > GetRowCount())
	...
	pTxtSch->GetCollapsableBlocksInfo(nTextRow, arCoBlocks, &m_fcCollapsable.ptrTBStart);
...
LRESULT CXTPSyntaxEditCtrl::OnGetText(WPARAM wBufferSize, LPARAM lpBuffer)
{
	...
	UINT uTextSize = (UINT)uBufferB/sizeof(TCHAR) - sizeof(TCHAR);
	uTextSize = max(0, uTextSize);
	uTextSize = min(uTextSize, (UINT)wBufferSize);

	ASSERT(uTextSize == _tcslen(pDest));
...
//      }
	...
	case VK_RIGHT:
		...
		if (!bCtrlKey)
		{
			...
			if (m_nCurrentCol > nTextLen && (nCurDocRow < GetRowCount()) && !m_bVirtualSpace)
			{
				MoveCurrentVisibleRowDown(1);
				m_nCurrentCol = 1;
			}
			else if (m_nCurrentCol <= nTextLen || m_bVirtualSpace)
			{
				m_nCurrentCol++;
			}
...
void CXTPSyntaxEditCtrl::SetCurCaretPos(int nTextRow, int nDispCol, BOOL bRowColNotify, BOOL bEnsureVisible)
{
	...
	BOOL bVirtSpace = m_bVirtualSpace || m_nAutoIndentCol;
	m_DrawTextProcessor.SetCaretPos(this, szCaret, max(0, nDispRow - 1), nCol,
									!m_bFocused, bVirtSpace);

	m_nDispCol = nCol + 1;
	m_nCurrentCol = m_DrawTextProcessor.DispPosToStrPos(nDispRow - 1, m_nDispCol-1, m_bVirtualSpace) + 1;
									...
									int *pDispRow, int *pDispCol)
{
	...
	BOOL bRet = m_DrawTextProcessor.ColFromXPos(nDispRow0, pt.x, nDispCol0, m_bVirtualSpace);
	if (!bRet)
	{
		...
		else if (pt.x > m_DrawTextProcessor.GetTextRect().right)
		{
			...
			m_DrawTextProcessor.ColFromXPos(nDispRow0, nX1, nDispCol0, m_bVirtualSpace);
		}
	}
	int nCol0 = m_DrawTextProcessor.DispPosToStrPos(nDispRow0, nDispCol0, m_bVirtualSpace);
	// to align to TABs positions
	nDispCol0 = m_DrawTextProcessor.StrPosToDispPos(nDispRow0, nCol0, m_bVirtualSpace);
...
void CXTPSyntaxEditCtrl::SetFontIndirect(LPLOGFONT pLogfont, BOOL bUpdateReg/*=FALSE*/)
{
	...
	{
		//m_wndToolTip.SetFont(GetPaintManager()->GetFontToolTip());
...
void CXTPSyntaxEditCtrl::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == TIMER_SELECTION_ID)
	{
		...
		int nNewRow = 0, nNewCol = 0, nNewDispRow = 0, nNewDispCol = 0;
		RowColFromPoint(ptCursor, &nNewRow, &nNewCol, &nNewDispRow, &nNewDispCol);
...
//                  nNewRow, nNewCol, nNewDispRow, nNewDispCol);
			...
			SetCurrentDocumentRow(nNewRow);
			m_nDispCol =  m_Selection.GetEnd_disp().nCol;
			m_nCurrentCol = m_Selection.GetEnd_str().nCol;
...
BOOL CXTPSyntaxEditCtrl::Select(int nRow1, int nDispCol1, int nRow2, int nDispCol2, BOOL bRedraw)
{
	...
	m_nDispCol = m_Selection.GetNormalEnd_disp().nCol;
	m_nCurrentCol = m_Selection.GetNormalEnd_str().nCol;
...
BOOL CXTPSyntaxEditCtrl::ReplaceSel(LPCTSTR szNewText)
{
	if (m_Selection.IsSelExist())
	{
		...
		m_nCurrentCol = m_Selection.GetNormalStart_str().nCol; //CalcAbsCol(GetLineText(m_Selection.GetNormalStart_disp().nLine), m_nDispCol);
	}
...
void CXTPSyntaxEditCtrl::CalculateEditbarLength(CDC* pDC)
{
	...
	if (GetLineNumbers())
	{
		...
		int nRowNumLen = (int)log10l(nMaxPageRow) + 1;
...
BOOL CXTPSyntaxEditCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_wndAutoComplete.IsActive())
	{
		return (BOOL)m_wndAutoComplete.SendMessage(WM_MOUSEWHEEL,
						MAKEWPARAM(nFlags, zDelta), MAKELPARAM(pt.x, pt.y));

		//CXTPWindowRect rcWndAC(&m_wndAutoComplete);
		//if (rcWndAC.PtInRect(pt))
		//  return (BOOL)m_wndAutoComplete.SendMessage(WM_MOUSEWHEEL, MAKEWPARAM(nFlags, zDelta), MAKELPARAM(pt.x, pt.y));
		//else
		//  m_wndAutoComplete.Hide();
	}
