...
void CXTPSyntaxEditPaintManager::DrawLineNumber(CDC *pDC, const CRect& rcRect, int nTextRow, CXTPSyntaxEditCtrl* pEditCtrl)
{
	...
	if (!pDC->IsPrinting())
		pDC->FillSolidRect(&rcRect, m_clrValues.crLineNumberBack);
			...
			DWORD dwType, int nTextRow, CXTPSyntaxEditCtrl* pEditCtrl)
{
	...
	UNREFERENCED_PARAMETER(pEditCtrl);
											  ...
											  int nLine, CXTPSyntaxEditCtrl* pEditCtrl)
{
	...
	while (pos)
	{
		...
		if (!pDC->IsPrinting())
		{
			...
			{
				static CString strSpace(_T(' '), 500);

				UpdateTextFont(pEditCtrl, txtBlk.lf);
				CXTPFontDC fontDC2(pDC, GetFontText());
				...
				if (bInfinitSelEnd && nSelStart <= nTextLenC)
				{
					...
					rcSel.left = max(rcSel.left, rcTextLine.left);
				...
				else
				{
					...
					if (nSelStart > nTextLenC)
					{
						...
						nSpaceLen = min(nSelStart - nTextLenC /*- 1*/, strSpace.GetLength());
						drawTxtProc.DrawRowPart(pDC, nLine, strSpace, nSpaceLen);
					}
					else
					{
						nSelStart2 = nTextLenC;
					}

					pDC->SetTextColor(crHiliteText);
					pDC->SetBkColor(crHiliteBack);

					nSpaceLen = min(nSelEnd - nSelStart2, strSpace.GetLength());
					if (bInfinitSelEnd)
						nSpaceLen = strSpace.GetLength();
					drawTxtProc.DrawRowPart(pDC, nLine, strSpace, nSpaceLen);
				}
			}
		}
