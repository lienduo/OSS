	...
	for (pTxt = pcszText; nTextLen_rest; )
	{
		...
		if (nFit > 0 && nFit <= nTextLen_rest)
		{
			...
			for (i = 0; i < nFit; i++)
			{
				...
				if ((nFlags & DT_WORDBREAK) && FIND_S(strSeps, *p_prev, 0) >= 0)
				{
					nFit_wb = i+1;
...
void CXTPSyntaxEditDrawTextProcessor::SetCaretByPoint(CWnd* pWnd, const CPoint& pt, const CSize& szSize,
						 int& rnRow, int& rnCol, BOOL bVirtualSpace)
{
	BOOL bHitRes = HitTest(pt, rnRow, rnCol, bVirtualSpace);
	VERIFY(bHitRes);

	SetCaretPos(pWnd, szSize, rnRow, rnCol);
}

void CXTPSyntaxEditDrawTextProcessor::SetCaretPos(CWnd* pWnd, const CSize& szSize,
						 int nRow, int& rnCol, BOOL bHideCaret, BOOL bVirtualSpace)
{
	...
	if (m_rcTextRect.PtInRect(CPoint(nCaretX, nRowY)) && !bHideCaret)
	{
