...
BOOL CXTPSyntaxEditSelection::_IsIntersectSel(BOOL bStr, int nTextRow, int nCol1, int nCol2)
{
	...
	{
		// infinit sel by end
		if (m_pOwnerCtrl->IsEnabledVirtualSpace())
		{
	...
	if (nTextRow >= normStart.nLine && nTextRow <= normEnd.nLine)
	{
		if (bBlockSelectionMode)
		{
			...
			if (m_pOwnerCtrl)
				...
				m_pOwnerCtrl->IsEnabledVirtualSpace());
	...
	if (nTextRow >= normStart.nLine && nTextRow <= normEnd.nLine)
	{
		...
		else
		{
			...
			else
			{
				...
				if (m_pOwnerCtrl)
				{
					nPos = m_pOwnerCtrl->GetDrawTextProcessor().DispPosToStrPos(nDispLine, nDispPos - 1,
						m_pOwnerCtrl->IsEnabledVirtualSpace());

					if (pbInfinitSelEnd)
						*pbInfinitSelEnd = m_pOwnerCtrl->IsEnabledVirtualSpace();
				}

				int nRowTextLen = m_pOwnerCtrl->GetEditBuffer()->GetLineTextLengthC(nTextRow);
				nPos = max(nPos, nRowTextLen);
				return nPos;
