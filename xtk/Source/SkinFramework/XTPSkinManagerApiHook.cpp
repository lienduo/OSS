...
BOOL CXTPSkinManagerApiHook::IsSystemWindowModule(WNDPROC lpWndProc)
{
	...
	static XTP_MODULEINFO miUser32 = {(LPVOID)-1, 0, 0};
	...
	static XTP_MODULEINFO miComCtrl32  = {(LPVOID)-1, 0, (LPVOID)-1};

	if (miComCtrl32.lpBaseOfDll == (LPVOID)-1)
	{
...
BOOL WINAPI CXTPSkinManagerApiHook::OnHookAdjustWindowRectEx(LPRECT lpRect, DWORD dwStyle, BOOL bMenu, DWORD dwExStyle)
{
	...
	if (bResult && ((dwStyle & WS_CAPTION) == WS_CAPTION) &&
		lpRect && (XTPSkinManager()->GetApplyOptions() & xtpSkinApplyMetrics))
	{
