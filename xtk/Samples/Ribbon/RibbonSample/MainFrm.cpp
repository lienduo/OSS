...
BOOL CMainFrame::CreateStatusBar()
{
	...
	CXTPStatusBarPane* pPane = m_wndStatusBar.AddIndicator(111, 0);
	pPane->SetText(_T("Page 1 of 1"));
	pPane->SetButton();
	pPane->SetMargins(7, 0, 5, 0);
	pPane->BestFit();

	pPane = m_wndStatusBar.AddIndicator(112, 1);
	pPane->SetText(_T("Words: 0"));
	pPane->SetButton();
	pPane->SetMargins(7, 0, 5, 0);
	pPane->BestFit();

	pPane = m_wndStatusBar.AddIndicator(113, 2);
	...
	m_wndStatusBar.SetRibbonDividerIndex(3);
	return TRUE;
}
...
BOOL CMainFrame::CreateRibbonBar()
{
	...
	CXTPControlPopupColor* pPopupColor = (CXTPControlPopupColor*)pGroupFont->Add(new CXTPControlPopupColor, ID_TEXT_COLOR);
	...
	pColorBar->GetControls()->Add(new CXTPControlColorSelector(), ID_TEXT_COLOR_SELECTOR);
	...
	pPopupColor = (CXTPControlPopupColor*)pGroupFont->Add(new CXTPControlPopupColor(), ID_TEXT_HIGHLIGHTCOLOR);
	...
	pGroupShowHide->Add(xtpControlCheckBox, ID_VIEW_THUMBNAILS);
	pGroupShowHide->Add(xtpControlCheckBox, ID_VIEW_ACTINBAR);
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	...
	if (lpCreateControl->nID >= ID_OPTIONS_STYLEBLUE && lpCreateControl->nID <= ID_OPTIONS_STYLEAQUA)
	{		
