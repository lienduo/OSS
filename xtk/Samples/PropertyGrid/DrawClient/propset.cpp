...
static LPVOID CountPrefixedStringW(LPCWSTR lpsz)
{
	...
	if (lp)
	{
		*lp = cb;
		...
		wcscpy_s( (LPWSTR)(lp+1), ((int)cb * sizeof(WCHAR) + sizeof(DWORD)) -1, lpsz );
#else
	...
	size_t cbCharSize)
{
	...
	size_t cbResult = 0;     // Number of bytes in pbResult
	...
	size_t cbCopy;           // Number of bytes to copy
	...
	while (nReps--)
	{
		...
		MEMCPY_S(pbResult + cbResult, (LPBYTE)&cchCopy, sizeof(DWORD));
		MEMCPY_S(pbResult + cbResult + sizeof(DWORD), pbBuffer, cbCopy);
...
BOOL CProperty::Set( const  LPVOID pVal )
{
	...
	MEMCPY_S( m_pValue, pValue, (int)cbValue );
...
#else // _UNICODE
	MEMCPY_S(entry.sz, (LPCSTR)strName, (size_t)entry.hdr.cb);
#endif // _UNICODE
...
BOOL CPropertySection::WriteNameDictToStream( IStream* pIStream )
{
	...
	while (pos != NULL)
	{
		...
		if (! WriteNameDictEntry(pIStream, (DWORD)(DWORD_PTR)pvID, strName))
			return FALSE;
	}
