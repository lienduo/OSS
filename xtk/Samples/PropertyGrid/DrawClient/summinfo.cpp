...
LPCSTR tcstocs(LPCTSTR lpctStr) {                       // typed char (WCHAR) to CHAR
	...
	wcstombs_s(&ConvertedChars, strTemp, 1024, lpctStr, 1024);
#else
...
{
	ULONG count;
	SCANF_S((LPCTSTR)GetRevNum(), _T("%lu"), &count);
	count++;
	TCHAR buff[20];
...
CString CSummInfo::GetEditTime()
{
	...
	TCHAR buff[20];
	SPRINTF_S(buff, _countof(buff), _T("%lu min"), editMinutes);
	return CString(buff);
}
...
CString CSummInfo::GetNumPages()
{
	...
	if (pNumPages != NULL)
	{
		SPRINTF_S(buff, _countof(buff), _T("%lu"), *pNumPages);
		return CString(buff);
	}
	else
		return CString(_T(""));
...
CString CSummInfo::GetNumWords()
{
	...
	if (pNumWords != NULL)
	{
		SPRINTF_S(buff, _countof(buff), _T("%lu"), *pNumWords);
		return CString(buff);
	}
	else
		return CString(_T(""));
...
CString CSummInfo::GetNumChars()
{
	...
	if (pNumChars != NULL)
	{
		SPRINTF_S(buff, _countof(buff), _T("%lu"), *pNumChars);
		return CString(buff);
	}
	else
		return CString(_T(""));
...
CString CSummInfo::GetSecurity()
{
	...
	if (pSecurity != NULL)
	{
		SPRINTF_S(buff, _countof(buff), _T("%lu"), *pSecurity);
		return CString(buff);
	}
	else
		return CString(_T(""));
