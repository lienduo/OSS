...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWnd)
	...
	ON_COMMAND_EX_RANGE(XTP_IDC_LANG_ARABIC, XTP_IDC_LANG_UKRAINIAN, OnLangaugeFile)
	ON_UPDATE_COMMAND_UI_RANGE(XTP_IDC_LANG_ARABIC, XTP_IDC_LANG_UKRAINIAN, OnUpdateLangaugeFile)
...
void CMainFrame::OnCustomize()
{
	...
	if(pCommandBars != NULL)
	{
		...
		pageKeyboard.AddCategories(IDR_MULTILTYPE);
...
void CMainFrame::OnInitCommandsPopup(CXTPPopupBar* pCommandBar)
{
		...
		pCommandBar->GetControl(0)->GetID() == ID_LANGAUGE_NOFOUND)
	{
		...
		for (UINT nCmdUI = XTP_IDC_LANG_ARABIC; nCmdUI <= XTP_IDC_LANG_UKRAINIAN; nCmdUI++)
		{
			WORD wLangID = m_langManager.GetLangID(nCmdUI);
			LANGUAGE_DLL langDll;

			if (!m_langManager.GetLangMap().Lookup(wLangID, langDll))
				continue;
...
void CMainFrame::OnUpdateLangaugeFile(CCmdUI* pCmdUI)
{
	...
	if (!IsUnicodeAndLayoutSupport())
	{
		switch (pCmdUI->m_nID)
		{
		case XTP_IDC_LANG_ARABIC:
		case XTP_IDC_LANG_HEBREW:
			{
				pCmdUI->Enable(FALSE);
				return;
			}

		case XTP_IDC_LANG_CHINESE_PRC:
		case XTP_IDC_LANG_CHINESE_TAIWAN:
		case XTP_IDC_LANG_GREEK:
		case XTP_IDC_LANG_JAPANESE:
		case XTP_IDC_LANG_KOREAN:
		case XTP_IDC_LANG_THAI:
		case XTP_IDC_LANG_RUSSIAN:
		case XTP_IDC_LANG_UKRAINIAN:
			{
				if (wLangID != GetSystemDefaultLangID())
				{				
					pCmdUI->Enable(FALSE);
					return;
				}
