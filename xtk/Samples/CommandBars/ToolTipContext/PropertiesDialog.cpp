...
CString CreateHTMLTool(CString strCaption, CString strDesc)
{
	...
	strHTML.Format(_T("<table cellspacing=4 cellpadding=0 border=0><tr><td><b>%s</b></td></tr>")
		...
		_T("<td><b>Press F1 for more help</b></td></tr></table></td></tr></table>"),
			(LPCTSTR)strCaption, (LPCTSTR)strDesc, (LPCTSTR)lpszModule);
...
void CPropertiesDialog::UpdateCommandBarToolTips()
{
	...
	if (pCommandBars->GetToolTipContext()->GetStyle() == xtpToolTipHTML)
	{
...
void CPropertiesDialog::UpdateControls()
{
	...
	GetDlgItem(IDC_CHECK_SHOWIMAGE)->EnableWindow(!bSystemStyle && (m_nToolTipStyle != xtpToolTipRTF && m_nToolTipStyle != xtpToolTipHTML));
	GetDlgItem(IDC_CHECK_SHOWBORDER)->EnableWindow(!bSystemStyle);
	
	GetDlgItem(IDC_CHECK_SHOWDESCRIPTION)->EnableWindow((m_nToolTipStyle != xtpToolTipRTF && m_nToolTipStyle != xtpToolTipHTML));
