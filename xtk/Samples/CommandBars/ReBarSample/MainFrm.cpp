...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	pWorkspaceBar->ShowExpandButton(FALSE);
	pWorkspaceBar->EnableDocking(xtpFlagStretched);
	pWorkspaceBar->EnableCustomization(FALSE);

	m_wndReBar.AddToolBar(pWorkspaceBar, RBBS_BREAK);

	m_wndClient.Attach(this);
	m_wndClient.SetAutoTheme(FALSE);
	m_wndClient.SetPaintManager(new CExplorerTabPaintManager());
	m_wndClient.SetFlags(xtpWorkspaceShowCloseSelectedTab | xtpWorkspaceHideClose | xtpWorkspaceHideArrows);
	
	CXTPControlTabWorkspace* pControl = (CXTPControlTabWorkspace*)pWorkspaceBar->GetControls()->Add(new CXTPControlTabWorkspace());
	pControl->SetFlags(xtpFlagControlStretched);
	
	m_wndClient.SetTabWorkspace(pControl);


	LoadCommandBars(_T("CommandBars3"));
	m_wndReBar.LoadState( _T("RebarState3") );
...
void CMainFrame::OnClose()
{
	...
	m_wndReBar.SaveState( _T("RebarState3") );
...
void CMainFrame::OnThemeStandard()
{
	...
	XTPPaintManager()->m_bSelectImageInPopupBar = TRUE;
	XTPPaintManager()->m_bThickCheckMark = TRUE;
