...
BOOL CTaskPanelView::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	cs.style |= LVS_ICON|LVS_AUTOARRANGE|WS_TABSTOP;
...
void CTaskPanelView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if (nChar == VK_TAB)
	{
		HWND hWnd = ::GetNextDlgTabItem(::GetParent(m_hWnd), m_hWnd, FALSE);
		if (hWnd)
			::SetFocus(hWnd);
		return;
	}
