...
void CPageKeyboard::OnListChange() 
{
	...
	HACCEL hAccelTable = m_pShortcutManager->GetDefaultAccelerator();
	ASSERT(hAccelTable);
	...
	for (int i = 0; i < nAccelSize; i ++)
	{
		...
		{
			CString str;
			CXTPShortcutManager::CKeyHelper helper (&lpAccel [i], m_pShortcutManager);
			helper.Format (str);
...
void CPageKeyboard::OnAssignChange() 
{
	...
	const ACCEL* pAccel = m_wndAssign.GetAccel();
	...
	for (int i = 0; i < nAccelSize; i ++)
	{
		if (CXTPShortcutManager::CKeyHelper::EqualAccels(&lpAccel[i], pAccel))
		{
			...
			{
				if (m_arrMenu[j].nID == lpAccel [i].cmd)
				{		  
...
{		
	...
	lpAccel[nIndex].cmd = (WORD)nID;

	m_pShortcutManager->UpdateAcellTable(lpAccel, nAccelSize + (nIndex == nAccelSize ? 1 : 0));

	m_cmbShortcuts.ResetContent();

	BOOL bFound = FALSE;
	
	for (i = 0; i < nAccelSize + 1; i++)
	{
		if (lpAccel[i].cmd == nID)
		{
			CString str;
...
void CPageKeyboard::OnButtonRemove() 
{
	...
	if (j >= 0 && j < nAccelSize)
	{
		...
		m_pShortcutManager->UpdateAcellTable(lpAccel, nAccelSize - 1);
	}
