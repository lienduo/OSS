...
void CPaneAccelerators::RefreshAccels(const HACCEL& hAccelTable)
{
	...
	for (int i = 0; i < m_nAccelSize; i ++)
	{
		...
		CString strKey = GetMainFrame()->GetCommandBars()->GetShortcutManager()->Format(&m_lpAccel[i], NULL);
...
void CPaneAccelerators::Refresh(BOOL bForceAll /*= FALSE*/)
{
	if (bForceAll || (m_pActiveFrame != GetMainFrame()->GetActiveEmbeddedFrame()))
	{
		...
		if (m_pActiveFrame)
		{
			HACCEL hAccel = GetActiveAccelTable();
			RefreshAccels(hAccel);
		}
		else
		{
			m_wndListCtrl.DeleteAllItems();
...
void CPaneAccelerators::Edit(int nIndex)
{
	...
	if (ap.DoModal() == IDOK)
	{
		...
		SetActiveAccelTable(CreateAcceleratorTable(m_lpAccel, m_nAccelSize));
...
HACCEL CPaneAccelerators::GetActiveAccelTable()
{
	ASSERT(m_pActiveFrame);
	return m_pActiveFrame->GetCommandBars()->GetShortcutManager()->GetDefaultAccelerator();
}

void CPaneAccelerators::SetActiveAccelTable(HACCEL hAccelTable)
{
	ASSERT(m_pActiveFrame);
	m_pActiveFrame->GetCommandBars()->GetShortcutManager()->SetDefaultAccelerator(hAccelTable);
}
...
void CPaneAccelerators::Delete(int nIndex)
{
	...
	SetActiveAccelTable(CreateAcceleratorTable(m_lpAccel, m_nAccelSize - 1));
...
void CPaneAccelerators::Insert()
{
	...
	if (ap.DoModal() == IDOK)
	{
		HACCEL hAccelTable = GetActiveAccelTable();
