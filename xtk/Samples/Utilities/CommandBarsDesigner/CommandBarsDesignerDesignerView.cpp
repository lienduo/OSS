...
BEGIN_MESSAGE_MAP(CCommandBarsDesignerDesignerView, CView)
	...
	ON_COMMAND(ID_COMMANDBARS_MENUGRABBER, OnCommandbarsMenugrabber)
	//}}AFX_MSG_MAP
...
{
	CXTPCommandBars* pCommandBars = m_pFrame->GetCommandBars();	
	BOOL bCustomizeMode = !pCommandBars->IsCustomizeMode();
	
	pCommandBars->SetCustomizeMode(bCustomizeMode);
	pCommandBars->m_bDesignerMode = bCustomizeMode;
}
...
void CCommandBarsDesignerDesignerView::OnCommandbarsMenugrabber() 
{
	CXTPCommandBars* pCommandBars = m_pFrame->GetCommandBars();
	if (!pCommandBars)
		return;
