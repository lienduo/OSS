...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	pImageManager->SetMaskColor(RGB(0, 0xFF, 0));
	...
	pImageManager->SetIcons(IDB_BITMAP_PANEICONS, nIDCommandsIcons, _countof(nIDCommandsIcons), CSize(16, 16));

	if (pImageManager->IsAlphaIconsSupported())
	{
		pImageManager->SetIcons(IDR_MAINFRAME, IDR_TOOLBAR_ALPHA);
	}
...
void CMainFrame::OnClose()
{
	...
	layoutNormal.Save(_T("StandardLayout"));
...
{
	if (lpCreateControl->nID == ID_DESIGN_MODE )
	{
		lpCreateControl->buttonStyle= xtpButtonIconAndCaption;
		return TRUE;
	}

	if (lpCreateControl->nID == ID_FILE_NEW && lpCreateControl->bToolBar)
	{
		CXTPControlPopup* pControl = CXTPControlPopup::CreateControlPopup(xtpControlSplitButtonPopup);

		CMenu mnu;
		mnu.LoadMenu(IDR_MENU_NEW);
		pControl->SetCommandBar(mnu.GetSubMenu(0));
