...
void CCalendarDemoView::OnInitialUpdate() 
{
	...
	//---------------------------------------------------------------------------
	OnOfficeThemeChanged(xtpThemeRibbon);
...
void CCalendarDemoView::OnOfficeThemeChanged(int nTheme)
{
	CXTPCalendarControl::CUpdateContext updateContext(&GetCalendarCtrl(), xtpCalendarUpdateAll);

	CMainFrame* pWnd = (CMainFrame*)AfxGetMainWnd();
	CXTPDatePickerControl& wndDatePicker = pWnd->m_dlgDatePicker.m_wndDatePicker;
	...
	if (nTheme == xtpThemeRibbon)
	{
		...
		CXTPDatePickerThemeOffice2007* pDPTheme2007 = new CXTPDatePickerThemeOffice2007();
		if (pDPTheme2007)		
			wndDatePicker.SetTheme(pDPTheme2007);		
	}
	else
	{
		GetCalendarCtrl().SetTheme(NULL);
		wndDatePicker.SetTheme(NULL);
	}
