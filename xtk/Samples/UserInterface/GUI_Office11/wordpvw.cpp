...
void CWordPadView::OnPenLens()
{
	...
	if (pCorrectWriting != NULL)
	{
		...
		{		
			...
			CString strSelText = GetRichEditCtrl().GetSelText();
			strcpy_s(pBuf, nCnt, T2A((LPTSTR)(LPCTSTR)strSelText));
#else
...
LRESULT CWordPadView::OnPrinterChangedMsg(WPARAM, LPARAM)
{
	CDC dc;
...
void CWordPadView::ShowContextMenu(CPoint point)
{
	ClientToScreen(&point);

	((CXTPFrameWnd*)AfxGetMainWnd())->GetCommandBars()->
		TrackPopupMenu(IDR_TEXT_POPUP, TPM_RIGHTBUTTON, point.x, point.y);
}

void CWordPadView::OnRButtonDown(UINT /*nFlags*/, CPoint point)
{
