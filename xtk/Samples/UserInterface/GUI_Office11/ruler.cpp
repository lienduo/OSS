	...
	m_tabItem(IDB_RULER_TAB)
{
	...
	LOGFONT lf;
	MEMCPY_S(&lf, &theApp.m_lf, sizeof(LOGFONT));
	lf.lfHeight = -8;
	lf.lfWidth = 0;
...
{
	pDC->FillSolidRect(CXTPClientRect(this), XTPPaintManager()->GetXtremeColor(XPCOLOR_3DFACE));
...
void CRulerBar::DrawFace(CDC& dc)
{
	...
	dc.FillSolidRect(CRect(nPageWidth + 2, 2, nPageEdge - 1, HEIGHT-2), XTPPaintManager()->GetXtremeColor(XPCOLOR_3DSHADOW));
	...
	dc.FillSolidRect(rcClient, XTPPaintManager()->GetXtremeColor(XPCOLOR_3DSHADOW));
