...
CSize CGnomeTheme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	...
	{
		if (!pControl->GetWrap())
			pDC->FillSolidRect(rcRow.left + 4, rcControl.top - 4, rcRow.Width() - 8, 1, RGB(161, 161, 161));
		else
			pDC->FillSolidRect(rcRow.right + 2, rcRow.top + 4, 1,  rcRow.Height() - 8, RGB(161, 161, 161));
	} else
	{
		if (!pControl->GetWrap())
			pDC->FillSolidRect(rcControl.left - 4, rcRow.top + 4, 1, rcRow.Height() - 8, RGB(161, 161, 161));
		else
			pDC->FillSolidRect(rcRow.left + 4, rcRow.top - 4, rcRow.Width() - 8, 1, RGB(161, 161, 161));
	}
	return 0;
}
