...
LRESULT CMainFrame::OnDockingPaneNotify(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam == XTP_DPN_PANEMENUCLICK)
	{
		...
		ShowDockingPanePopupMenu(pPopupPane, pClick->pt);
	...
	{
		CXTPDockingPane* pPopupPane = 0;
		CXTPDockingPaneBase* pContainer = (CXTPDockingPaneBase*)lParam;
		if (pContainer->GetType() == xtpPaneTypeTabbedContainer)
			pPopupPane = ((CXTPDockingPaneTabbedContainer*)pContainer)->GetSelected();
		else if (pContainer->GetType() == xtpPaneTypeDockingPane)
			pPopupPane = (CXTPDockingPane*)pContainer;

		if (!pPopupPane) return TRUE;

		CPoint pos;
		GetCursorPos(&pos);

		ShowDockingPanePopupMenu(pPopupPane, pos);
		return TRUE;
	}
	return 0;
}
